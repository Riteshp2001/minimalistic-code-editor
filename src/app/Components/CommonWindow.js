  ...
  return (
    <div className="p-2 rounded-lg shadow-lg w-full">
      <FileContext.Provider
        value={{ html, css, js, theme, setHtml, setCss, setJs, setTheme }}
      >
        <FilesViewer fileNames={fileNames} setFileNames={setFileNames} />
        <ThemeSelector />
        <SplittingWindow setFileNames={setFileNames} fileNames={fileNames} />
      </FileContext.Provider>
    </div>
  );
}
    ...
    <div className="relative  overflow-x-auto w-[95%] mx-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-white mb-4">
      <div className="flex p-3 mb-3 flex-row-reverse justify-end">
        {fileNames.map((file) => (
          <span
            key={file.id}
            className={`mx-2 p-2 flex items-center whitespace-nowrap rounded-xl shadow-lg transition duration-300 ease-in-out border-2 transform hover:-translate-y-1 cursor-pointer ${
              file.isSelected
                ? `scale-105 border-blue-300 bg-blue-700`
          ...
          >
            ...
            onClick={() => setSelectedFile(file.id)}
              className="cursor-pointer text-center w-30 overflow-hidden inline-block transition-all duration-300 hover:underline bg-gradient-to-r from-transparent to-white mask bg-cover z-10"
            >
              {file.name}
            </span>
            ...
            <span onClick={() => deleteSelectedFile(file.id)}>
              <svg
                ...
                height="24px"
                viewBox="0 0 24 24"
                className="inline cursor-pointer transition-all hover:scale-110 z-10"
                fill="none"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
